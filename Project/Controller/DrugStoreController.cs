        ...
        public void Sale()
        {
            ...
            if (result)
            {
                ...
                if (drugstores!=null)
                {
                    foreach (var drugstore in drugstores)
                    {
                        ConsoleHelper.WriteTextWithColor(ConsoleColor.Magenta, $"  DrugStores Id:{drugstore.ID} DrugStores name-{drugstore.Name}" +
                            $" DrugStores address-{drugstore.Address} DrugStores contact number {drugstore.ContactNumber}");
                ...
                if (drugStore != null)
                {
                    var dbDrugs = _drugRepository.GetAll(d => d.DrugStore.ID == Id);
                    if (dbDrugs.Count>0)
                    {
